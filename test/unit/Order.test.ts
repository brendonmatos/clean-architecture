...
test("should be able to add item to order", function() {
    const order = new Order({cpf: "783.511.060-11"})
    order.addEntry(new OrderEntry({product: new Product("1", "creme de barbear", 0, {x: 1, y:1, z:1}, 20_00), quantity:2}))
})
...
test("should sum all items from order", function() {
    const order = new Order({cpf: "783.511.060-11"})
    order.addEntry(new OrderEntry({product: new Product("1", "creme de barbear", 0, {x: 1, y:1, z:1}, 20_00), quantity:2}))
    order.addEntry(new OrderEntry({product: new Product("1", "creme de barbear", 0, {x: 1, y:1, z:1}, 10_00), quantity:2}))
    order.addEntry(new OrderEntry({product: new Product("1", "creme de barbear", 0, {x: 1, y:1, z:1}, 15_00), quantity:2}))
